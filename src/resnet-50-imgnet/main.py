...
from data.imagenet import Data
...
def main():
    ...
    model_t = import_module(f'model.{ARCH}').__dict__[args.target_model](wbit = args.bitW, abit = args.abitW, num_classes = args.num_classes).to(device)
...
def train(args, loader_train, models, optimizers, epoch, block_bits = None):
    ...
    f = f'{args.class_num_dir}result{args.lt_gamma}.json'
    
    with open(f, "r") as outfile:
        class_num_data_res = json.load(outfile)
    
    sample_num_per_cls = list(class_num_data_res.values())
    ...
    for i, (inputs, targets) in enumerate(loader_train, 1):
        ...
        count_mat = torch.zeros([inputs.size(0), 1000])
        ...
        error_t = loss(output_t, targets, features) 
...
def test(args, loader_test, model_t, epoch = 0):
    ...
    with torch.no_grad():
        for i, (inputs, targets) in enumerate(loader_test, 1):
            ...
            logits, _ = model_t(inputs.to(device), [1])
