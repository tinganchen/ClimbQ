...
class Block(nn.Module):
    ...
    def forward(self, x, p):
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.act_q1(out, p)
        out = self.relu(out)
        
        out = self.conv2(out)
        out = self.bn2(out)
        out = self.act_q2(out, p)
        out = self.relu(out)
        
        out = self.conv3(out)
        out = self.bn3(out)
        out = self.act_q3(out, p)
...
class MobileNetV2(nn.Module):
    ...
    def __init__(self, block, wbit, abit, stage, num_classes=100):
...
def mobile_v2(bitW, abitW, stage, **kwargs):
    return MobileNetV2(Block, wbit = bitW, abit = abitW, stage = stage, **kwargs)
