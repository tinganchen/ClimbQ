...
class PreActBlock_conv_Q(nn.Module):
  ...
  def forward(self, x, p):
    ...
    if self.skip_conv is not None:
      ...
      shortcut = self.act_skip_q(self.skip_bn(shortcut), p)
    else:
      shortcut = x
    
    out = self.conv0(x)
    out = F.relu(self.act_q0(self.bn0(out), p))
    
    out = self.conv1(out)
    out = self.act_q1(self.bn1(out), p)
...
class PreActResNet(nn.Module):
  ...
  def forward(self, x, p):
    out = self.conv0(x)
    out = self.bn(out)
    out = self.act_q0(out, p)
    out = F.relu(out)
    
    for layer in self.layers:
      out = layer(out, p)
    
    self.out = self.avgpool(out)
    feat = self.out.view(self.out.size(0), -1)
    out = self.logit(feat)
    return out, feat

def resnet8_quant(bitW, abitW, stage, num_classes=10):
  return PreActResNet(PreActBlock_conv_Q, [1, 1, 1], bitW, abitW, stage, num_classes=num_classes)
